...
Meteogram.prototype.tooltipFormatter = function (tooltip) {
    ...
    ret += '<b>' + this.symbolNames[index]
	.replace('Clear', 'Ясно')
	.replace('sky', '')
	.replace('Fair', 'Ясно')
	.replace('Partly cloudy', 'Переменная облачность')
	.replace('Cloudy', 'Облачно')
	.replace('Rain showers', 'Ливень')
	.replace('Light rain', 'Небольшой дождь') 
	.replace('Heavy rain', 'Сильный дождь')
	.replace('showers', '')	
	+ '</b>';
    ...
    // Add wind
    ret += '<tr><td style="vertical-align: top">\u25CF Ветер</td><td style="white-space:nowrap">' + this.windDirectionNames[index]
	.replace('East-northeast', 'Северно-восточный')
	.replace('Northeast', 'Cеверно-восточный')	
	.replace('North', 'Северный')
	.replace('East', 'Восточный')
	.replace('east', '-восточный')
	.replace('West', 'Западный')
	.replace('South', 'Южно')
	+'<br>' + this.windSpeedNames[index]
	.replace('Light breeze', 'Слабый ветер')
	.replace('Light air', 'Слабый ветер')
	.replace('Gentle breeze', 'Легкий ветер')
	.replace('Moderate breeze', 'Умеренный ветер')
	+ ' (' +
        Highcharts.numberFormat(this.windSpeeds[index], 1) + ' м/с)</td></tr>';
...
Meteogram.prototype.windArrow = function (name) {
    ...
    level = $.inArray(name, ['Штиль', 'Тихий ветер', 'Легкий ветер', 'Небольшой ветер', 'Умеренный ветер',
        'Прохладный ветер', 'Сильный ветер', 'Ураган', 'Ветер', 'Штормовой ветер', 'Шторм',
...
Meteogram.prototype.getTitle = function () {
    return 'Погода в ' + this.xml.location.name.replace('Kyiv', 'Киеве') + ', ' + this.xml.location.country.replace('Ukraine', 'Украина');
};
...
Meteogram.prototype.getChartOptions = function () {
    ...
    return {
        ...
        credits: {
            text: '<a href="http://vesti-ukr.com" target="_blank">vesti-ukr.com</a>',
...
Meteogram.prototype.parseYrData = function () {
    ...
    if (!xml || !xml.forecast) {
        $('#loading').html('<i class="fa fa-frown-o"></i> Ошибка загрузки');
        return;
    }
...
$(function () { // On DOM ready...
    ...
    if (!location.hash) {
        var place = 'Ukraine/Kiev/Kyiv';
        //place = 'France/Rhone-Alpes/Val_d\'Isere~2971074';
        //place = 'Norway/Sogn_og_Fjordane/Vik/Malset';
